...
try {
  customElements.define("coles-knob", class ColesKnob extends WebAudioControlsWidget {
    constructor() {
      super();
    }
    ...
    connectedCallback() {
      ...
      <style>
        ...
        .knob-container {
          position: relative;
          width: var(--diameter, 64px);
          height: var(--diameter, 64px);
          border-radius: 50%;
        ...
        .knob-notch {
          position: absolute;
          width: 4px;
          height: var(--notch-length, calc(var(--diameter, 64px) / 2 - 6px));
          background-color: var(--notch-color, #e00);
          top: calc(50% - var(--notch-length)); /* Adjust top based on notch length */
          left: 50%;
          transform-origin: bottom center; /* Origin at the bottom to rotate from center */
          transform: rotate(var(--angle, 0deg));
        }
      </style>
      <div class='knob-container' tabindex='1' touch-action='none'>
        <div class='knob-notch'></div>
      </div>
      <div class='webaudioctrl-tooltip'></div>
      `;
      
      this.elem = root.querySelector('.knob-container');
      this.notch = root.querySelector('.knob-notch');
      ...
      this.outline = this.getAttr("outline", 1);
      this.sensitivity = this.getAttr("sensitivity", 1);
      this.valuetip = this.getAttr("valuetip", 0);
    ...
    redraw() {
      ...
      let angle = 270 * ((this.value - this.min) / range - 0.5);
      this.notch.style.setProperty('--angle', `${angle}deg`);
    }
  ...
  console.log("coles-knob already defined");
}
