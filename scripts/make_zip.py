...
def  main():
  ...
  installer = "\installer\IPlugWebEmbed Installer.exe"
   
  if demo:
    installer = "\installer\IPlugWebEmbed Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugWebEmbed manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugWebEmbed-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugWebEmbed-v" + FULLVERSIONSTR + "-win-demo.zip"
